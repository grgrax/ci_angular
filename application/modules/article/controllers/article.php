...
class article extends Frontend_Controller
{
	...
	function __construct()
	{
		try {
			...
			$this->load->helper(array('article'));
			
			$this->data['actions']=article_m::actions();
			$this->data['link']=base_url().self::MODULE;
			$this->data['rows']=$this->article_m->read_all($this->article_m->count_rows());
	...
	function index($offset=0)
	{
		try {
			$per_page=25;
			$total=$this->article_m->count_rows();
			$this->data['rows']=$this->article_m->read_all($per_page,$offset);
			if($total>$per_page){
	...
	function add()
	{
		try {
			if($this->input->post())
			{
				$rules=$this->article_m->get_rules();
				...
				if($this->form_validation->run($this)===TRUE)
				{
					...
					// }
					$this->article_m->create_row($this->data['insert_data']);
	...
	function edit($slug=FALSE)
	{
		try {
			...
			if($this->input->post())
			{
				$rules=$this->article_m->get_rules(array('name'));
				$name=$this->input->post('name');
				$name_rule=array(
					'field'=>'name',
					'label'=>'Category Fund Name',
					'rules'=>"trim|required|xss_clean|route|is_article_name_unique[$id]",
					// 'rules'=>"trim|required|xss_clean|is_column_name_unique[$id|article/article_m]",
					);
				...
				if($this->form_validation->run($this)===TRUE)
				{
					...
					$this->article_m->update_row($id,$this->data['update_data']);
	...
	function publish($slug=NULL){
		try{
			...
			$response=$this->get_data(array('slug'=>$slug));
			if(!$response['success']) throw new Exception($response['data'], 1);
			$this->data=array('status'=>article_m::PUBLISHED);
			$this->article_m->update_row($response['data']['id'],$this->data);
	...
	function unpublish($slug=NULL){
		try{
			$response=$this->get_data(array('slug'=>$slug));
			if(!$response['success']) throw new Exception($response['data'], 1);
			$this->data=array('status'=>article_m::UNPUBLISHED);
			$this->article_m->update_row($response['data']['id'],$this->data);
	...
	function delete($slug=NULL){
		try{
			...
			$response=$this->get_data(array('slug'=>$slug));
			if(!$response['success']) throw new Exception($response['data'], 1);
			$this->data=array('status'=>article_m::DELETED);
			$this->article_m->update_row($response['data']['id'],$this->data);
	...
	function get_data($param){
		...
		$row=$this->article_m->read_rows_by($param,1);
		if($row) {
